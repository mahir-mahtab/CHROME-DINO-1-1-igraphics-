...
# include <stdio.h>
# include <stdlib.h>
#include <map> // For storing textures by ID
...
std::map<int, GLuint> imageTextures; // Map to store textures with an ID
int nextImageId = 1; // To assign unique IDs to images

// Function to load an image and create a texture
int iLoadImage(const char* filename) {
    int width, height, n;
    stbi_set_flip_vertically_on_load(1);
    unsigned char* image = stbi_load(filename, &width, &height, &n, 4);
    ...
    imageTextures[nextImageId] = texture; // Store the texture
    return nextImageId++;
}

// Function to render an image texture
void iShowImage(int x, int y, char filename[]) {
    int width, height, n;
    stbi_set_flip_vertically_on_load(true); // Correct image orientation
    unsigned char* image = stbi_load(filename, &width, &height, &n, 4);
...
// Function to free texture resources
void iDeleteImage(int id) {
    if (imageTextures.find(id) != imageTextures.end()) {
        glDeleteTextures(1, &imageTextures[id]);
        imageTextures.erase(id);
    }
}
